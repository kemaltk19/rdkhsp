...
// değerler kullanılır (invoice_service.go convertToDefaultCurrency). Satırları burada
...
const taxTotal = computed(() => Number(props.invoice?.tax_total) || 0)
const grandTotal = computed(() => Number(props.invoice?.total) || 0)

const exchangeRatesInfo = computed(() => {
  const items = props.invoice?.items || [];
  const rates = new Map<string, number>();
  for (const item of items) {
    if (item.currency && item.currency.toUpperCase() !== props.invoice?.currency?.toUpperCase()) {
  ...
  <div class="fp-doc">
    ...
    <div class="fp-meta">
      <div class="fp-mc">
        ...
        <div class="fp-mv fp-mv-total"><Money :value="grandTotal.toString()" :currency="invoice?.currency" /></div>
      </div>
      <div class="fp-ms"></div>
      <div class="fp-mc">
        <div class="fp-ml">FATURA NO</div>
        <div class="fp-mv uppercase-input">{{ invoice?.number || 'TASLAK' }}</div>
      </div>
      <div class="fp-ms"></div>
      <div class="fp-mc">
        <div class="fp-ml">FATURA TARİHİ</div>
        <div class="fp-mv">{{ formatDate(invoice?.date) }}</div>
      </div>
      <div class="fp-ms"></div>
      <div class="fp-mc">
        <div class="fp-ml">VADE TARİHİ</div>
        <div class="fp-mv">{{ formatDate(invoice?.due_date) }}</div>
      </div>
      <div class="fp-ms"></div>
      <div class="fp-mc">
        <div class="fp-ml">DÖVİZ</div>
        <div class="fp-mv font-bold">{{ invoice?.currency }}</div>
      </div>
    </div>
    ...
    <div class="fp-tbl-wrap">
      <table class="fp-tbl">
        ...
        <tbody>
          <tr v-for="(item, idx) in invoice?.items" :key="idx" class="ftr">
            ...
            <td class="ftd r"><Money :value="item.unit_price" :currency="item.currency || invoice?.currency" /></td>
            <td class="ftd c">%{{ item.tax_rate }}</td>
            <td class="ftd r ftd-linetot"><Money :value="lSub(item).toString()" :currency="item.currency || invoice?.currency" /></td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="fp-bot">
      <div class="fp-bot-l">
        ...
        <div class="fp-notes whitespace-pre-wrap">{{ invoice?.note || '-' }}</div>
        <div class="fp-sig">
      ...
      <div class="fp-bot-r">
        <div class="fp-tots">
          <div class="fp-trow"><span>Ara Toplam</span><Money :value="subtotal.toString()" :currency="invoice?.currency" /></div>
          <div class="fp-trow">
            <span class="whitespace-nowrap">İndirim / Ek (-/+)</span>
            <span class="font-semibold"><Money :value="(invoice?.discount_total || 0).toString()" :currency="invoice?.currency" /></span>
          </div>
          <div class="fp-trow"><span>KDV</span><Money :value="taxTotal.toString()" :currency="invoice?.currency" /></div>
          <div class="fp-tsep"></div>
          <div class="fp-tgrand">
            <span>GENEL TOPLAM</span>
            <span class="fp-tgval"><Money :value="grandTotal.toString()" :currency="invoice?.currency" /></span>
          </div>
          <template v-if="Number(invoice?.paid_total || 0) > 0">
            <div class="fp-tsep"></div>
            <div class="fp-trow fp-trow-paid">
              <span>Ödenen Tutar</span>
              <span class="fp-tgval-paid"><Money :value="invoice?.paid_total?.toString()" :currency="invoice?.currency" /></span>
            </div>
            <div class="fp-tgrand fp-tgrand-rem">
              <span>KALAN TUTAR</span>
              <span class="fp-tgval"><Money :value="(grandTotal - Number(invoice?.paid_total || 0)).toString()" :currency="invoice?.currency" /></span>
            </div>
          </template>
        ...
        <div class="fp-deco">{{ invoice?.type === 'sales' ? 'FATURA' : 'ALIŞ' }}</div>
      </div>
    </div>
...
.fp-trow { display: flex; justify-content: space-between; align-items: center; color: #475569; font-size: 13px; font-weight: 500; }
.fp-tsep { border-top: 1px solid #cbd5e1; margin: 4px 0; }
.fp-trow-paid { color: #10b981; font-weight: 700; margin-top: 4px; }
.fp-tgval-paid { font-family: 'SFProNumbers', monospace; font-size: 14px; }
.fp-tgrand { 
  display: flex; justify-content: space-between; align-items: center;
  background: #ecfeff; border: 1.5px solid #06b6d4; border-radius: 8px; padding: 10px 12px; gap: 16px; margin-top: 6px;
}
.fp-tgrand-rem { background: #fef2f2; border-color: #ef4444; margin-top: 6px; }
.fp-tgrand span:first-child { font-size: 11px; font-weight: 800; color: #0891b2; letter-spacing: 1px; }
.fp-tgrand-rem span:first-child { color: #ef4444; }
.fp-tgval { font-size: 18px; font-weight: 900; color: #0891b2; font-family: 'SFProNumbers', monospace; }
.fp-tgrand-rem .fp-tgval { color: #ef4444; }
.fp-deco {
