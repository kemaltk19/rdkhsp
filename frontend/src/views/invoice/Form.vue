...
const form = ref({
  ...
  note: '',
  status: 'draft',
  process_stock: true, // false ise bu fatura stok hareketi yaratmaz (hizmet/gider)
  discount_total: '0',
  items: [] as Array<{
...
// Stok onayı yalnızca alışta görünür. Satışa geçilince gizli toggle'ın
// kapalı kalıp sessizce stok işlememesini önlemek için varsayılana (true) döndür.
watch(() => form.value.type, (t) => {
  if (t !== 'purchase') form.value.process_stock = true
})

// 'sent' durumunun etiketi fatura tipine göre değişir: satış müşteriye
// GÖNDERİLİR, alış tedarikçiden ALINIR (DB değeri her ikisinde de 'sent').
const statusOptions = computed(() => [
  { label: 'Taslak', value: 'draft' },
  { label: form.value.type === 'purchase' ? 'Alındı' : 'Gönderildi', value: 'sent' },
])
...
onMounted(async () => {
  ...
  try {
    ...
    if (!isEdit.value) {
      ...
      form.value.due_date = getCurrentCompanyDatetimeLocal(15)
      addRow()
      try {
        const n = await settingsStore.fetchSetting('invoice_footer_note')
        if (n) form.value.note = n
      } catch {}
    } else {
      const id = route.params.id as string
      await invoiceStore.fetchInvoiceByID(id)
      if (invoiceStore.activeInvoice) {
        const inv = invoiceStore.activeInvoice
        if (inv.status !== 'draft') {
          toast.add({ severity: 'error', summary: 'Hata', detail: 'Sadece taslak faturalar düzenlenebilir.', life: 10000 })
          router.push('/invoices'); return
        }
        form.value = {
          cari_id: inv.cari_id, type: inv.type, number: inv.number,
          date: inv.date ? toCompanyDatetimeLocal(inv.date) : '',
          due_date: inv.due_date ? toCompanyDatetimeLocal(inv.due_date) : '',
          currency: inv.currency, note: inv.note || '', status: inv.status,
          process_stock: inv.process_stock !== false, // eski kayıtlar (undefined) -> true
...
const handleProductSelect = (item: any, event: any) => {
  ...
  if (p) {
    ...
    // Ürünün satış fiyatı KDV dahilse, fatura birim fiyatı KDV hariç olmalı:
...
// Backend invoice_service.go'daki convertToDefaultCurrency ile aynı mantık.
...
const cancel = () => router.push('/invoices')

const checkDuplicateInvoice = async () => {
  if (!form.value.cari_id) return null
  const newDate = new Date(form.value.date)
  const newYear = newDate.getFullYear()
  const newMonth = newDate.getMonth()

  await invoiceStore.fetchInvoices({ cari_id: form.value.cari_id, limit: 1000 })
  const existingInvoices = invoiceStore.invoices

  for (const inv of existingInvoices) {
    const invDate = new Date(inv.date)
    if (invDate.getFullYear() === newYear && invDate.getMonth() === newMonth && inv.status !== 'canceled' && inv.id !== route.params.id) {
      if (inv.items) {
        for (const existingItem of inv.items) {
          const existingProd = productStore.products.find(p => p.id === existingItem.product_id)
          const existingCatId = existingProd?.category_id

          for (const newItem of form.value.items) {
            const newProd = newItem.product_id ? productStore.products.find(p => p.id === newItem.product_id) : null
            const newCatId = newProd?.category_id

            if (newCatId && existingCatId && newCatId === existingCatId) {
              const isSameAmount = new Decimal(existingItem.unit_price).eq(new Decimal(newItem.unit_price))
              if (isSameAmount) {
                return {
                  invoiceNumber: inv.number,
                  productName: newProd?.name,
                  amount: existingItem.unit_price
                }
              }
            }
          }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (dup) {
      const confirmSave = confirm(`Mükerrer Belge Uyarısı:\nAynı ay içerisinde bu cari için aynı kategoride benzer bir işlem tespit edildi (Fatura No: ${dup.invoiceNumber}, Tutar: ${dup.amount}). Yine de kaydetmek istiyor musunuz?`)
      if (!confirmSave) {
        loading.value = false
        return
      }
  ...
  try {
    const payload = {
      ...
      due_date: toBackendDate(form.value.due_date),
    ...
    let savedInvoice
    if (isEdit.value) {
      savedInvoice = await invoiceStore.updateInvoice(route.params.id as string, payload)
      toast.add({ severity: 'success', summary: 'Başarılı', detail: 'Fatura güncellendi', life: 10000 })
    } else {
      savedInvoice = await invoiceStore.createInvoice(payload)
      toast.add({ severity: 'success', summary: 'Başarılı', detail: 'Fatura oluşturuldu', life: 10000 })
    }

    // Backend, mail gönderimi başarısız olursa hata döner ve fatura "sent" olamaz;
    // bu yüzden bu mesaj sadece backend'den dönen gerçek statü "sent" ise gösterilir.
    if (savedInvoice?.status === 'sent') {
      const recipient = savedInvoice.type === 'purchase' ? 'tedarikçiye' : 'müşteriye'
      toast.add({ severity: 'info', summary: 'E-posta', detail: `Fatura ${recipient} e-posta ile gönderildi.`, life: 10000 })
    }
    
    // Stoklar ve faturalar değiştiği için bildirimleri yenile
  ...
  <div class="fp-page">
    ...
    <div class="fp-bar">
      <div class="fp-bar-l">
        ...
        <span class="fp-bar-title">{{ isEdit ? 'Faturayı Düzenle' : 'Yeni Fatura' }}</span>
      </div>
      <div class="fp-bar-r">
    ...
    <form id="fp-form" @submit.prevent="handleSubmit" class="fp-doc">
      ...
      <div class="fp-head">
        <div class="fp-head-l">
          ...
          <div class="fp-to-label">FATURA KESİLEN</div>
      ...
      <div class="fp-meta">
        ...
        <div class="fp-ms"></div>
        <div class="fp-mc">
          <div class="fp-ml">FATURA TARİHİ</div>
          <input type="datetime-local" v-model="form.date" class="fp-md" required :disabled="loading" />
        </div>
        <div class="fp-ms"></div>
        <div class="fp-mc">
          <div class="fp-ml">VADE TARİHİ</div>
          <input type="datetime-local" v-model="form.due_date" class="fp-md" required :disabled="loading" />
        </div>
        <div class="fp-ms"></div>
        <div class="fp-mc">
        ...
        <!-- Stok işleme onayı yalnızca alış faturasında gösterilir -->
        <div v-if="form.type === 'purchase'" class="fp-ms"></div>
        <div v-if="form.type === 'purchase'" class="fp-mc">
          <div class="fp-ml">STOK İŞLENSİN</div>
          <div class="fp-stock-toggle">
            <ToggleSwitch v-model="form.process_stock" :disabled="loading" inputId="process_stock" />
            <span class="fp-stock-hint">{{ form.process_stock ? 'Evet' : 'Hayır (hizmet/gider)' }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="fp-bot">
        ...
        <div class="fp-bot-r">
          ...
          <div class="fp-deco">{{ form.type === 'sales' ? 'FATURA' : 'ALIŞ' }}</div>
        </div>
      </div>
...
.fp-stock-toggle { display: flex; align-items: center; gap: 8px; }
.fp-stock-hint { font-size: 12px; font-weight: 600; color: #64748b; }
.fp-mi {
